...
export function getRepr(originalBits: BitString, bits: BitString, refs: Cell[], levelMask: LevelMask, level: number, type: CellType) {
    ...
    // Write descriptors
    let reprCursor = 0;
    repr[reprCursor++] = getRefsDescriptor(refs, levelMask.apply(level).value, type);
